...
class ModelCode extends CCodeModel
{
    ...
    public function generateRules($table)
    {
        ...
        if ($integers !== array())
            $rules[] = "array('" . implode(', ', $integers) . "', 'NumberValidator', 'integerOnly' => true)";
        if ($numerical !== array())
            $rules[] = "array('" . implode(', ', $numerical) . "', 'NumberValidator')";
        if ($length !== array())
        {
            foreach ($length as $len => $cols)
            {
                if($len > 0)
                {
                    $rules[] = "array('" . implode(', ', $cols) . "', 'StringValidator', 'max' => $len)";
                }
                else
                {
                    $rules[] = "array('" . implode(', ', $cols) . "', 'StringValidator')";
                }
            }
        }
        if ($safe !== array())

...
class ErrorHandler extends CApplicationComponent
{
    ...
    protected function handleException($exception)
    {
        ...
        if ($app instanceof CWebApplication) {
            //check if sequence failed
            if (strpos($exception->getMessage(), 'duplicate key value') !== false
                    && strpos($exception->getMessage(), 'js_error_log_pkey') === false) {
                sql('SELECT adjust_seqs(\'public\');')->execute();
            }
            ...
            $realFile = null;
            $realLine = null;
            $realLineContent = null;
            $realFileSet = false;

            if ($this->isOurFile($fileName)) {
                $realFile = $fileName;
                $realFileSet = true;
                $realLineContent = $this->getLine($fileName, $errorLine);
                $realLine = $errorLine;
            }
            ...
            foreach ($trace as $i => $t) {
                ...
                if (!$realFileSet) {
                    if ($trace[$i]['file'] != 'unknown' && $this->isOurFile($trace[$i]['file'])) {
                        $realFile = $trace[$i]['file'];
                        $realLine = $trace[$i]['line'];
                        $realLineContent = $this->getLine($trace[$i]['file'], $trace[$i]['line']);
                        $realFileSet = true;
                    }
                }
            ...
            $this->_error = $data = array(
                ...
                'getParams' => print_r($_GET, true),
                'realFile' => $realFile,
                'realLine' => $realLine,
                'realLineContent' => $realLineContent,
                'session' => print_r(isset($_SESSION) ? $_SESSION : array(), true),
    ...
    protected function handleError($event)
    {
        ...
        if($this->isOurFile($event->file))
        {
            $realFile = $event->file;
            $realFileSet = TRUE;
            $realLine = $event->line;
            $realLineContent = $this->getLine($event->file, $event->line);
        }
...
//            else if (YII_DEBUG)
    ...
    public function isOurFile($file)
    {
        return strpos($file, 'framework') === FALSE;
    }

    public function getLine($filePath, $line)
    {
        $file = file($filePath);
        return $file[$line - 1];
    }
